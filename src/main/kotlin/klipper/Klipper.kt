...
// region Typealiases
//Change CInt to Int for 32bit integer instead of 64bit ints. This may
//improve performance but coordinate values are limited to the range +/- 46340
//Note: performance shouldn't improve on 64bit processor architecture.
internal typealias CInt = Long // Int
...
internal const val ZERO = 0L // Change to "0" if CInt is Int
internal const val LO_RANGE: CInt = 0x3FFFFFFF // Change to "0x7FFF" if CInt is Int
internal const val HI_RANGE: CInt = 0x3FFFFFFFFFFFFFFFL // Change to "0x7FFF" if CInt is Int
...
data class DoublePoint(
    ...
    infix fun set(other: DoublePoint) {
        x = other.x
        y = other.y
    }
}
...
data class IntPoint(
    ...
    infix fun set(other: IntPoint) {
        x = other.x
        y = other.y
    }

    fun set(x: CInt, y: CInt) {
        this.x = x
        this.y = y
    }

    internal fun isOnLineSegment(linePt1: IntPoint, linePt2: IntPoint, useFullRange: Boolean): Boolean {
        return if (useFullRange) {
            (this == linePt1 || this == linePt2
                    || ((x > linePt1.x == x < linePt2.x) && (y > linePt1.y == y < linePt2.y)
                    && (x - linePt1.x) mul (linePt2.y - linePt1.y)
                    == (linePt2.x - linePt1.x) mul (y - linePt1.y)))
    ...
    internal fun isOnPolygon(pp: OutPt, useFullRange: Boolean): Boolean {
        var pp2 = pp
        while (true) {
...
internal data class LocalMinima(
        var y: CInt = 0,
        var leftBound: TEdge? = null,
        var rightBound: TEdge? = null,
        var next: LocalMinima? = null
)

internal data class Scanbeam(
        var y: CInt = 0,
        var next: Scanbeam? = null
)

internal data class Maxima(
        var x: CInt = 0,
        var next: Maxima? = null,
        var prev: Maxima? = null
)

internal data class Join(
        var outPt1: OutPt? = null,
        var outPt2: OutPt? = null,
        var offPt: IntPoint = IntPoint()
)

internal data class HorzDirection(
        var left: CInt,
        var right: CInt,
...
internal inline val Double.isNearZero: Boolean
    get() = (this > -TOLERANCE) && (this < TOLERANCE)

fun round(value: Double): CInt = if (value < 0) (value - 0.5).toLong() else (value + 0.5).toLong()

// Overflow-safe multiply
internal infix fun CInt.mul(other: CInt) {
    BigInteger.valueOf(this) * BigInteger.valueOf(other)
}

internal fun slopesEqual(e1: TEdge, e2: TEdge, useFullRange: Boolean): Boolean {
    return if (useFullRange) {
        e1.delta.y mul e2.delta.x == e1.delta.x mul e2.delta.y
    } else {
...
internal fun slopesEqual(pt1: IntPoint, pt2: IntPoint, pt3: IntPoint, useFullRange: Boolean): Boolean {
    return if (useFullRange) {
        (pt1.y - pt2.y) mul (pt2.x - pt3.x) == (pt1.x - pt2.x) mul (pt2.y - pt3.y)
    } else {
...
internal fun slopesEqual(pt1: IntPoint, pt2: IntPoint, pt3: IntPoint, pt4: IntPoint, useFullRange: Boolean): Boolean {
    return if (useFullRange) {
        (pt1.y - pt2.y) mul (pt3.x - pt4.x) == (pt1.x - pt2.x) mul (pt3.y - pt4.y)
    } else {
        (pt1.y - pt2.y) * (pt3.x - pt4.x) == (pt1.x - pt2.x) * (pt3.y - pt4.y)
    }
}

internal fun pt2IsBetweenPt1AndPt3(pt1: IntPoint, pt2: IntPoint, pt3: IntPoint) = when {
...
private fun getDx(pt1: IntPoint, pt2: IntPoint): Double {
    return if (pt1.y == pt2.y)
        HORIZONTAL
    else
        (pt2.x - pt1.x).toDouble() / (pt2.y - pt1.y)
}
...
private fun intersectNodeSort(node1: IntersectNode, node2: IntersectNode): Int {
    ...
    return (node2.pt.y - node1.pt.y).toInt()
}
...
private fun parseFirstLeft(firstLeft: OutRec?): OutRec? {
    var newFirstLeft = firstLeft
    while (newFirstLeft != null && newFirstLeft.pts == null)
        newFirstLeft = newFirstLeft.firstLeft
    return newFirstLeft
}
...
private fun slopesNearCollinear(
    ...
    return (if (Math.abs(pt1.x - pt2.x) > Math.abs(pt1.y - pt2.y)) {
        when {
            ...
            else -> distanceFromLineSqrd(pt3, pt1, pt2)
        }
    } else {
        when {
...
fun minkowskiSum(pattern: Path, path: Path, pathIsClosed: Boolean): Paths {
    return minkowski(pattern, path, true, pathIsClosed).also { paths ->
        Clipper().apply {
...
fun minkowskiDiff(poly1: Path, poly2: Path): Paths {
    return minkowski(poly1, poly2, false, true).also { paths ->
        Clipper().apply {
            addPaths(paths, PolyType.Subject, true)
        }.execute(ClipType.Union, paths, PolyFillType.NonZero, PolyFillType.NonZero)
    }
...
// region Path
// Path is actually TypeAlias of MutalbleList<IntPoint>
fun Path(vararg elements: IntPoint): Path = mutableListOf(*elements)
...
open class PolyNode(
    ...
    val contour: Path = Path()
    private var index = 0
    val children: ArrayList<PolyNode> = ArrayList()
...
class PolyTree : PolyNode() {
    ...
    val first: PolyNode? get() = children.firstOrNull()

    val total: Int
        get() {
            var result = allPolys.size
            //with negative offsets, ignore the hidden outer polygon ...
            if (result > 0 && children[0] != allPolys[0]) result--
            return result
        }
    ...
    fun clear() {
        allPolys.clear()
        (children as MutableList<PolyNode>).clear()
    }
}
// endregion PolyTree
...
internal class TEdge {
    ...
    internal fun init(polyType: PolyType) {
    ...
    internal fun init(eNext: TEdge, ePrev: TEdge, pt: IntPoint) {
        next = eNext
        prev = ePrev
        curr set pt
        outIdx = UNASSIGNED
    ...
    internal val isHorizontal: Boolean get() = delta.y == ZERO
    ...
    internal fun reverseHorizontal() {
        val tmp = top.x
        top.x = bot.x
        bot.x = tmp
    }
...
// IntersectNode Comparator
internal object IntersectNodeComparator : Comparator<IntersectNode> {
...
internal class OutRec {
    ...
    internal fun isRightOf(outRec2: OutRec): Boolean {
    ...
    internal fun calcArea(): Double = pts?.calcArea() ?: 0.0
}
// endregion OutRec
...
internal class OutPt(
    ...
    internal fun reversePolyPtLinks() {
        var pp1: OutPt
        var pp2: OutPt
        pp1 = this
        do {
    ...
    //See "The Point in Polygon Problem for Arbitrary Polygons" by Hormann & Agathos
    //http://citeseerx.ist.psu.edu/viewdoc/download?doi=10.1.1.88.5498&rep=rep1&type=pdf
    internal fun containsPoint(pt: IntPoint): Int {
        var op = this
        //returns 0 if false, +1 if true, -1 if pt ON polygon boundary
        var result = 0
        val startOp = op
    ...
    internal fun containsPoint(polygon: OutPt): Boolean {
        var op = polygon
        do {
    ...
    internal fun hasVertex(ip: IntPoint): Boolean {
        var pp2 = this
        do {
...
open class ClipperBase {
    ...
    private fun findNextLocMin(edge: TEdge): TEdge {
        var e = edge
        var e2: TEdge
        while (true) {
    ...
    private fun processBound(edge: TEdge, leftBoundIsForward: Boolean): TEdge {
        var e = edge
        var eStart: TEdge
        var result = e
        var horz: TEdge
        ...
        if (result.outIdx == SKIP) {
            //check if there are edges beyond the skip edge in the getBounds and if so
            ...
            if (e == result) {
                ...
                //there are more edges in the getBounds beyond result starting with edge
        ...
        if (leftBoundIsForward) {
            ...
            if (result.dx == HORIZONTAL && result.next.outIdx != SKIP) {
                //nb: at the top of a bounds, horizontals are added to the bounds
            ...
            if (e.dx == HORIZONTAL && e != eStart && e.bot.x != e.prev.top.x)
                e.reverseHorizontal()
            result = result.next //move to the edge just beyond current bounds
        } else {
            ...
            if (e.dx == HORIZONTAL && e != eStart && e.bot.x != e.next.top.x)
                e.reverseHorizontal()
            result = result.prev //move to the edge just beyond current bounds
        }
        return result
    }
    ...
    fun addPath(pg: Path, polyType: PolyType, closed: Boolean): Boolean {
        ...
        while (true) {
            ...
            //Compare their slopes to find which starts which bounds ...
            val locMin = LocalMinima().apply {
                next = null
                y = e.bot.y
                if (e.dx < e.prev.dx) {
    ...
    internal fun popLocalMinima(y: CInt): LocalMinima? {
        val prevLM = currentLM
    ...
    internal fun updateEdgeIntoAEL(edge: TEdge): TEdge {
        return edge.apply {
...
class Clipper(
    ...
    internal var intersectNodeComparator: Comparator<IntersectNode> = IntersectNodeComparator
    ...
    private fun insertMaxima(x: CInt) {
        ...
        if (maxima == null) {
            ...
            if (x == m.x) return  //ie ignores duplicates (& CG to cleanPolygon up newMax)
    ...
    private fun executeInternal(): Boolean {
        try {
            ...
            while (popScanbeam()?.let { topY = it } != null || localMinimaPending()) {
                processHorizontals()
                ghostJoins.clear()
    ...
    private fun insertLocalMinimaIntoAEL(botY: CInt) {
        ...
        while (popLocalMinima(botY).also { lm = it } != null) {
            val lb = lm!!.leftBound
    ...
    private fun insertEdgeIntoAEL(edge: TEdge, startEdge: TEdge?) {
        ...
        } else if (newStartEdge == null && e2InsertsBeforeE1(activeEdges, edge)) {
            edge.prevInAEL = null
            edge.nextInAEL = activeEdges
            activeEdges.prevInAEL = edge
            this.activeEdges = edge
        } else {
            if (newStartEdge == null) newStartEdge = activeEdges
            while (newStartEdge!!.nextInAEL?.let { !e2InsertsBeforeE1(it, edge) } ?: false) {
                newStartEdge = newStartEdge.nextInAEL
            }
            edge.nextInAEL = newStartEdge.nextInAEL
            newStartEdge.nextInAEL?.prevInAEL = edge
            edge.prevInAEL = newStartEdge
            newStartEdge.nextInAEL = edge
        }
    }
    ...
    internal fun popEdgeFromSEL(): TEdge? {
    ...
    private fun horzSegmentsOverlap(segment1a: CInt, segment1b: CInt, segment2a: CInt, segment2b: CInt): Boolean {
        var seg1a = segment1a
        var seg1b = segment1b
        var seg2a = segment2a
        var seg2b = segment2b
    ...
    private fun firstIsBottomPt(btmPt1: OutPt, btmPt2: OutPt): Boolean {
        ...
        if (Math.max(dx1p, dx1n) == Math.max(dx2p, dx2n) && Math.min(dx1p, dx1n) == Math.min(dx2p, dx2n))
            return btmPt1.calcArea() > 0 //if otherwise identical use getOrientation
        else
            return dx1p >= dx2p && dx1p >= dx2n || dx1n >= dx2p && dx1n >= dx2n
    }

    private fun getBottomPt(point: OutPt): OutPt {
        var pp = point
        var dups: OutPt? = null
        var p = pp.next
        while (p != pp) {
    ...
    private fun processHorizontals() {
        ...
        while (popEdgeFromSEL()?.let { horzEdge = it } != null) {
            processHorizontal(horzEdge!!)
        }
    }

    internal fun getHorzDirection(horzEdge: TEdge): HorzDirection {
        return if (horzEdge.bot.x < horzEdge.top.x) {
            HorzDirection(horzEdge.bot.x, horzEdge.top.x, Direction.LeftToRight)
        } else {
            HorzDirection(horzEdge.top.x, horzEdge.bot.x, Direction.RightToLeft)
        }
    }

    private fun processHorizontal(horizontalEdge: TEdge) {
        var hEdge = horizontalEdge
        var dir: Direction? = null
        var horzLeft: CInt = ZERO
        var horzRight: CInt = ZERO
        val IsOpen = hEdge.windDelta == 0

        getHorzDirection(hEdge).also {
            dir = it.direction
            horzLeft = it.left
            horzRight = it.right
        }
        ...
        if (currMax != null) {
            ...
            if (dir == Direction.LeftToRight) {
                while (currMax != null && currMax.x <= hEdge.bot.x)
                    currMax = currMax.next
                if (currMax != null && currMax.x >= eLastHorz.top.x)
                    currMax = null
            } else {
                while (currMax!!.next?.let { it.x < hEdge.bot.x } ?: false)
                    currMax = currMax.next
        ...
        {
            val isLastHorz = (hEdge == eLastHorz)
            var e = getNextInAEL(hEdge, dir!!)
            while (e != null) {
                ...
                if (currMax != null) {
                    if (dir == Direction.LeftToRight) {
                        while (currMax != null && currMax.x < e.curr.x) {
                            if (hEdge.outIdx >= 0 && !IsOpen)
                                addOutPt(hEdge, IntPoint(currMax.x, hEdge.bot.y))
                            currMax = currMax.next
                        }
                    } else {
                        while (currMax != null && currMax.x > e.curr.x) {
                ...
                if (e.curr.x == hEdge.top.x && hEdge.nextInLML?.let { e!!.dx < it.dx } ?: false) break

                if (hEdge.outIdx >= 0 && !IsOpen)  //note: may be done multiple times
                {
                    op1 = addOutPt(hEdge, e.curr)
                    var eNextHorz = sortedEdges
                    while (eNextHorz != null) {
                        if (eNextHorz.outIdx >= 0 &&
                                horzSegmentsOverlap(
                                        hEdge.bot.x,
                                        hEdge.top.x, eNextHorz.bot.x, eNextHorz.top.x
                                )
                        ) {
                ...
                if (e == eMaxPair && isLastHorz) {
                    if (hEdge.outIdx >= 0)
                        addLocalMaxPoly(hEdge, eMaxPair, hEdge.top)
                    deleteFromAEL(hEdge)
                ...
                if (dir == Direction.LeftToRight) {
                    val pt = IntPoint(e.curr.x, hEdge.curr.y)
                    intersectEdges(hEdge, e, pt)
                } else {
                    val pt = IntPoint(e.curr.x, hEdge.curr.y)
                    intersectEdges(e, hEdge, pt)
                }
                val eNext = getNextInAEL(e, dir!!)
                swapPositionsInAEL(hEdge, e)
                e = eNext
            } //end while(e != null)

            //Break out of loop if HorzEdge.NextInLML is not also horizontal ...
            if (!(hEdge.nextInLML?.isHorizontal ?: false)) break

            hEdge = updateEdgeIntoAEL(hEdge)

            if (hEdge.outIdx >= 0) addOutPt(hEdge, hEdge.bot)
        ...
        if (hEdge.outIdx >= 0 && op1 == null) {
            op1 = getLastOutPt(hEdge)
            var eNextHorz = sortedEdges
            while (eNextHorz != null) {
                if (eNextHorz.outIdx >= 0 &&
                        horzSegmentsOverlap(
            ...
            addGhostJoin(op1, hEdge.top)
        }

        if (hEdge.nextInLML != null) {
            if (hEdge.outIdx >= 0) {
                op1 = addOutPt(hEdge, hEdge.top)

                hEdge = updateEdgeIntoAEL(hEdge)

                if (hEdge.windDelta == 0) return
                //nb: HorzEdge is no longer horizontal here
                val ePrev = hEdge.prevInAEL
                val eNext = hEdge.nextInAEL
                if (ePrev != null && ePrev.curr.x == hEdge.bot.x &&
                        ePrev.curr.y == hEdge.bot.y && ePrev.windDelta != 0 &&
                        (ePrev.outIdx >= 0 && ePrev.curr.y > ePrev.top.y &&
                                slopesEqual(hEdge, ePrev, useFullRange))
                ) {
                    val op2 = addOutPt(ePrev, hEdge.bot)
                    addJoin(op1, op2, hEdge.top)
                } else if (eNext != null && eNext.curr.x == hEdge.bot.x &&
                        eNext.curr.y == hEdge.bot.y && eNext.windDelta != 0 &&
                        eNext.outIdx >= 0 && eNext.curr.y > eNext.top.y &&
                        slopesEqual(hEdge, eNext, useFullRange)
                ) {
                    val op2 = addOutPt(eNext, hEdge.bot)
                    addJoin(op1, op2, hEdge.top)
                }
            } else {
                hEdge = updateEdgeIntoAEL(hEdge)
            ...
            if (hEdge.outIdx >= 0) addOutPt(hEdge, hEdge.top)
            deleteFromAEL(hEdge)
        }
    }
        ...
        while (isModified && activeEdges != null) {
            ...
            while (e!!.nextInSEL != null) {
                ...
                if (e.curr.x > eNext.curr.x) {
                    var pt: IntPoint = intersectPoint(e, eNext)
                    if (pt.y < topY)
    ...
    private fun intersectPoint(edge1: TEdge, edge2: TEdge): IntPoint {
        val ip = IntPoint()
        val b1: Double
        val b2: Double
    ...
    internal fun getOverlap(a1: CInt, a2: CInt, b1: CInt, b2: CInt): Pair<CInt, CInt> {
        val left: CInt
        val right: CInt
        if (a1 < a2) {
            if (b1 < b2) {
    ...
    internal fun joinHorz(
            point1a: OutPt, point1b: OutPt, point2a: OutPt, point2b: OutPt,
        ...
        var op2a = point2a
        var op2b = point2b
        val dir1 = if (op1a.pt.x > op1b.pt.x) Direction.RightToLeft else Direction.LeftToRight
        val dir2 = if (op2a.pt.x > op2b.pt.x) Direction.RightToLeft else Direction.LeftToRight
        ...
        if (dir1 == Direction.LeftToRight) {
            while (op1a.next.pt.x <= pt.x &&
                    op1a.next.pt.x >= op1a.pt.x && op1a.next.pt.y == pt.y
            )
                op1a = op1a.next
            if (discardLeft && op1a.pt.x != pt.x) op1a = op1a.next
            op1b = dupOutPt(op1a, !discardLeft)
            if (op1b.pt != pt) {
                op1a = op1b
                op1a.pt set pt
                op1b = dupOutPt(op1a, !discardLeft)
        ...
        if (dir2 == Direction.LeftToRight) {
            while (op2a.next.pt.x <= pt.x &&
                    op2a.next.pt.x >= op2a.pt.x && op2a.next.pt.y == pt.y
            )
                op2a = op2a.next
            if (discardLeft && op2a.pt.x != pt.x) op2a = op2a.next
            op2b = dupOutPt(op2a, !discardLeft)
            if (op2b.pt != pt) {
                op2a = op2b
                op2a.pt set pt
                op2b = dupOutPt(op2a, !discardLeft)
    ...
    private fun joinPoints(j: Join, outRec1: OutRec, outRec2: OutRec): Boolean {
        ...
        if (isHorizontal && (j.offPt == op1.pt) && (j.offPt == op2.pt)) {
            ...
            getOverlap(op1.pt.x, op1b.pt.x, op2.pt.x, op2b.pt.x).also {
                left = it.first
                right = it.second
...
class ClipperOffset(
    ...
    companion object { // TODO maybe move these as constants
    ...
    fun addPath(path: Path, joinType: JoinType, endType: EndType) {
        ...
        if (lowest.x < 0)
            ...
            val ip = polyNodes.children[lowest.x.toInt()].contour[lowest.y.toInt()]
    ...
    private fun fixOrientations() {
        //fixup orientations of all closed paths if the getOrientation of the
        //closed path with the lowermost vertex is wrong ...
        if (lowest.x >= 0 && !polyNodes.children[lowest.x.toInt()].contour.orientation) {
            polyNodes.children.forEach { node ->
    ...
    fun execute(solution: PolyTree, delta: Double) {
        ...
        if (delta > 0) {
            ...
            if (solution.childCount == 1) {
                solution.children[0].let { outerNode ->
                    if (outerNode.childCount > 0) {
                        solution.children.ensureCapacity(outerNode.childCount)
                        solution.children[0] = outerNode.children[0].apply {
                            parent = solution
                        }
